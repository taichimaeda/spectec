...
\subsection{\K{SELECT}}
If we get \K{SELECT} from stack, then we have two cases: condition is true or false. \\
...
\end{lcverbatim}
We can do similarly with the case where \K{binop\_(valtype)} is \K{SUB} and \K{MUL}:
\begin{lcverbatim}
def $iadd(N, iN(N), iN(N)) : iN(N)
...
\end{lcverbatim}
Now, we can combine them by declaring a new function \K{binop} as follows:
{\color{red}
	\begin{lcverbatim}
def $iadd(N, iN(N), iN(N)) : iN(N)
def $isub(N, iN(N), iN(N)) : iN(N)
