...
(* Lemma Definition at: spec/wasm-1.0-test/A-soundness.watsup:388.1-393.39 *)
...
(* Lemma Definition at: spec/wasm-1.0-test/A-soundness.watsup:396.1-405.76 *)
...
(* Lemma Definition at: spec/wasm-1.0-test/A-soundness.watsup:396.1-405.76 *)
Lemma Step_read__preserves_call : forall (v_x : idx) (v_z : state) (v_ret : (option resulttype)) (v_lab : (list resulttype)) (v_valtype_1 : (list valtype)) (v_val_1 : (list val)) (v_ft : functype) (v_C' : context) (v_C : context) (v_f : frame) (v_s : store), ((v_C' = ({| context__TYPES := []; context__FUNCS := []; context__GLOBALS := []; context__TABLES := []; context__MEMS := []; context__LOCALS := []; context__LABELS := v_lab; context__RETURN := None |} ++ ({| context__TYPES := []; context__FUNCS := []; context__GLOBALS := []; context__TABLES := []; context__MEMS := []; context__LOCALS := []; context__LABELS := []; context__RETURN := v_ret |} ++ ({| context__TYPES := []; context__FUNCS := []; context__GLOBALS := []; context__TABLES := []; context__MEMS := []; context__LOCALS := (@app _ v_valtype_1 (context__LOCALS v_C)); context__LABELS := []; context__RETURN := None |} ++ v_C)))) -> ((Admin_instrs_ok v_s v_C' [(admininstr__CALL v_x)] v_ft) -> ((Module_instance_ok v_s (frame__MODULE v_f) v_C) -> ((v_val_1 = (frame__LOCALS v_f)) -> (List.Forall2 (fun v_val_1 v_valtype_1 => (Val_ok v_val_1 v_valtype_1)) (v_val_1) (v_valtype_1) -> (Admin_instrs_ok v_s v_C' [(admininstr__CALL_ADDR (lookup_total (fun_funcaddr v_z) v_x))] v_ft)))))).
Proof. Admitted.
