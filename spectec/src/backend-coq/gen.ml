open Il.Ast
open Util
open Source
open Case
let parens s = "(" ^ s ^ ")"
let curly_parens s = "{" ^ s ^ "}"
let square_parens s = "[" ^ s ^ "]"

(* Function prefix (To avoid same name clash on types) *)
let func_prefix = "fun_"

(* variable prefix (To avoid same name clash on types) *)
let var_prefix = "v_"

module IdSet = Set.Make(String)

let reserved_ids = ["N"; "in"; "In"; "()"; "tt"; "Import"; "Export"; "List"; "String"; "Type"; "list"; "nat"] |> IdSet.of_list

let gen_id' (s : text) = match s with
 | s when IdSet.mem s reserved_ids -> "reserved__" ^ s
 | s -> String.map (function
    | '.' -> '_'
    | '-' -> '_'
    | c -> c
    ) s

let gen_id (s : id) = gen_id' s.it

let rec gen_exp_id_used (e : exp) =
  match e.it with
    | VarE id -> [var_prefix ^ gen_id id] |> IdSet.of_list
    | UnE (_op, exp) -> gen_exp_id_used exp
    | BinE (_binop, exp1, exp2) -> IdSet.union (gen_exp_id_used exp1) (gen_exp_id_used exp2)
    | CmpE (_cmpop, exp1, exp2) -> IdSet.union (gen_exp_id_used exp1) (gen_exp_id_used exp2)
    | SliceE (exp1, exp2, exp3) -> IdSet.union (gen_exp_id_used exp1) (IdSet.union (gen_exp_id_used exp2) (gen_exp_id_used exp3))
    | UpdE (exp, _path, exp2) -> IdSet.union (gen_exp_id_used exp) (gen_exp_id_used exp2)
    | ExtE (exp1, _path, exp2) -> IdSet.union (gen_exp_id_used exp1) (gen_exp_id_used exp2)
    | StrE expfields -> List.fold_left IdSet.union IdSet.empty (List.map (fun (_, e) -> gen_exp_id_used e) expfields)
    | DotE (exp, _atom) -> gen_exp_id_used exp  
    | CompE (exp1, exp2) -> IdSet.union (gen_exp_id_used exp1) (gen_exp_id_used exp2)       
    | TupE exps -> List.fold_left IdSet.union IdSet.empty (List.map gen_exp_id_used exps)          
    | CallE (id, _args) -> [gen_id id] |> IdSet.of_list
    | IterE (exp, (_iter, _ids)) -> gen_exp_id_used exp 
    | OptE (Some exp) -> gen_exp_id_used exp  
    | TheE exp -> gen_exp_id_used exp             
    | ListE exps -> List.fold_left IdSet.union IdSet.empty (List.map gen_exp_id_used exps)          
    | CatE (exp1, exp2) -> IdSet.union (gen_exp_id_used exp1) (gen_exp_id_used exp2)    
    | CaseE (_mixop, exp) -> gen_exp_id_used exp     
    | SubE (exp, _typ1, _typ2) -> gen_exp_id_used exp
    | ProjE (exp, _n) -> gen_exp_id_used exp
    | UncaseE (exp, _mixop) -> gen_exp_id_used exp
    | LenE exp -> gen_exp_id_used exp
    | _ -> IdSet.empty


let gen_atom_total (a : atom) = 
  match a.it with
    | Atom id -> Some (gen_id' id)
    | _ -> None

let gen_atom (a : atom) = 
  match gen_atom_total a with
    | Some id -> id
    | None -> ""

let gen_mixop (m : mixop) =
  match m with
    | [{it = Atom a; _}]::tail when List.for_all ((=) []) tail -> gen_id' a
    | mixop -> String.concat "" (List.map (
        fun atoms -> String.concat "" (List.filter_map gen_atom_total atoms)) mixop
      )

let gen_iter_typ (it : iter) =
  match it with
    | Opt -> "option" 
    | (List | List1 | ListN _) -> "list"

let gen_typ_name (t : typ) =
  match t.it with
    | VarT (id, _) -> gen_id id
    | _ -> assert false

let gen_binop (b : binop) =
  match b with
    | AndOp -> " /\\ "
    | OrOp -> " \\/ "
    | AddOp _ -> " + "
    | SubOp _ -> " - "
    | ImplOp -> " -> "
    | MulOp _ -> " * "
    | DivOp _ -> " / "
    | ExpOp _ -> " ^ "
    | EquivOp -> " <-> "
let is_addop (b : binop) = 
  match b with
    | AddOp _ -> true
    | _ -> false
let gen_unop (u : unop) =
  match u with
    | NotOp -> "~"
    | PlusOp _ -> ""
    | MinusOp _ -> "0 - "
    | PlusMinusOp _ -> "0 - "
    | MinusPlusOp _ -> "0 - "

let gen_cmpop (c : cmpop) =
  match c with
    | EqOp -> " = "
    | NeOp -> " <> "
    | LtOp _ -> " < "
    | GtOp _ -> " > "
    | LeOp _ -> " <= "
    | GeOp _ -> " >= "

let get_num_from_exp (e : exp) =
  match e.it with
    | NatE nat -> nat
    | _ -> Z.zero

let rec gen_exp (is_match : bool) (e : exp) =
  match e.it with
    | VarE id -> var_prefix ^ gen_id id
    | BoolE true -> "true"
    | BoolE false -> "false"
    | NatE nat -> Z.to_string nat
    | TextE text -> "\"" ^ String.escaped text ^ "\""
    | UnE (op, exp) ->  parens (gen_unop op ^ (gen_exp is_match exp))
    | BinE (binop, exp1, exp2) -> let num2 = get_num_from_exp exp2 in 
      if is_match && is_addop binop && num2 <> Z.zero
      then (gen_succ (Z.to_int num2) exp1)
      else parens (gen_exp is_match exp1 ^ gen_binop binop ^ gen_exp is_match exp2)
    | CmpE (cmpop, exp1, exp2) -> parens (gen_exp is_match exp1 ^ gen_cmpop cmpop ^ gen_exp is_match exp2)
    | SliceE (_exp1, _exp2, _exp3) -> "8" (* TODO *)
    | UpdE (exp1, path, exp2) -> parens ("list_update " ^ parens (gen_exp is_match exp1) ^ " " ^ parens (gen_path path is_match exp1) ^ " " ^ parens (gen_exp is_match exp2))
    | ExtE (exp1, path, exp2) -> parens ("list_extend " ^ parens (gen_path path is_match exp1) ^ " " ^ parens (gen_exp is_match exp2))
    | StrE expfields -> "{| " ^ String.concat "; " (List.map (fun (a, exp) -> 
      gen_typ_name e.note ^ "__" ^ gen_atom a ^ " := " ^ gen_exp false exp) expfields) 
      ^ " |}"
    | DotE (exp, atom) -> parens (gen_typ_name exp.note ^ "__" ^ gen_atom atom ^  " " ^ gen_exp is_match exp)
    | CompE (exp, exp2) -> parens (gen_exp false exp ^ " ++ " ^ gen_exp false exp2)
    | TupE [] -> ""
    | TupE exps -> String.concat " " (List.map (gen_exp is_match) exps)
    | CallE (id, args) -> parens (func_prefix ^ gen_id id ^ " " ^ gen_call_args args)
    | IterE (exp, (_iter, _ids)) -> gen_exp is_match exp
    | OptE None -> "None"
    | OptE (Some exp) -> "Some " ^ gen_exp is_match exp  
    | TheE exp -> "the" ^ parens (gen_exp is_match exp)             
    | ListE exps -> (match exps with
      | [] -> "[]"
      | [e] -> if is_match then gen_exp is_match e else square_parens (gen_exp is_match e)
      | _ -> square_parens (String.concat ";" (List.map (gen_exp false) exps)))
    | LenE exp -> "List.length(" ^ gen_exp is_match exp ^ ")"
    | CatE (exp1, exp2) -> let op = if is_match then " :: " else " ++ " in
      let exp1_option = (match exp1.it with
        | IterE (_, (Opt, _)) -> "option_to_list "
        | _ -> ""
      ) ^ gen_exp is_match exp1 
      in
      let exp2_option = (match exp2.it with
        | IterE (_, (Opt, _)) -> "option_to_list "
        | _ -> ""
      ) ^ gen_exp is_match exp2
      in      
      parens (exp1_option ^ op ^ exp2_option)   
    | IdxE (exp1, exp2) -> parens ("lookup_total(" ^ gen_exp is_match exp1 ^ ") (" ^ gen_exp is_match exp2 ^ ")" )
    | CaseE (mixop, exp1) -> parens (gen_typ_name e.note ^ "__" ^ gen_mixop mixop ^ " " ^ gen_exp is_match exp1)
    | SubE (exp, _typ1, _typ2) -> gen_exp is_match exp
    | ProjE (_exp, n) -> string_of_int n
    | UncaseE (_exp, _mixop) -> "2" (* TODO *)

and gen_tup_exp (exp : exp) = 
  match exp.it with
    | TupE exps -> parens (String.concat " , " (List.map gen_tup_exp exps))
    | _ -> gen_exp false exp

and gen_numtyp (nt : numtyp) =
  match nt with
    | NatT -> "nat"
    | IntT -> "Z"
    | RatT -> "float"
    | RealT -> "float"

and gen_typ (t: typ) =
  match t.it with
    | VarT (id, args) -> gen_id id ^ (gen_call_args args)
    | NumT nt -> gen_numtyp nt
    | TextT -> "string"
    | BoolT -> "bool"
    | TupT [] -> "Type"
    | TupT typs -> String.concat " * " (List.map (fun (_, t) -> gen_typ t) typs)
    | IterT (typ, iter) -> gen_iter_typ iter ^ " " ^ parens (gen_typ typ)

and gen_typ_args (t : typ) =
  match t.it with
    | TupT typs -> String.concat " " (List.map (fun (e, t) -> parens (gen_exp true e ^ " : " ^ gen_typ t)) typs)
    | _ -> parens (gen_typ t)

and gen_relation_param_types (t : typ) =
  match t.it with
    | TupT typs -> String.concat " -> " (List.map (fun (_, t) -> gen_typ t) typs)
    | _ -> gen_typ t

and gen_bind_typ (b : bind) =
  match b.it with 
    | ExpB (_, typ, _) -> gen_typ typ
    | TypB id -> gen_id id

and gen_bind (b : bind) =
  match b.it with 
    | ExpB (id, typ, iters) -> let iter_typs = if iters == [] then "" else String.concat " " (List.map gen_iter_typ iters) ^ " " in
      parens (var_prefix ^ gen_id id ^ " : " ^ iter_typs ^ gen_typ typ)
    | TypB id -> gen_id id

and gen_arg (a : arg * bind) =
  match a with
    | ({it = ExpA exp; _},{it = ExpB (_, typ, _); _}) -> Some (parens (gen_exp false exp ^ " : " ^ gen_typ typ))
    | ({it = TypA typ; _},{it = TypB _; _}) -> Some (curly_parens (gen_typ typ))
    | _ -> None

and gen_inductive_args (binds : bind list) (args : arg list) =
  match args with
    | [] -> ""
    | _ -> String.concat " " (List.filter_map gen_arg (List.combine args binds))

and gen_arg_name (is_match : bool) (a : arg) = 
  match a.it with
    | ExpA exp -> Some (gen_exp is_match exp)
    | _ -> None

and gen_call_args (args : arg list) =
  match args with
  | [] -> ""
  | _ -> String.concat " " (List.map parens (List.filter_map (gen_arg_name false) args))

and gen_match_args (args : arg list) = 
  match args with
    | [] -> ""
    | _ -> parens (String.concat ", " (List.filter_map (gen_arg_name true) args))

and gen_succ (n : int) (e : exp) : text =
  match n with
    | 0 -> gen_exp false e 
    | m -> "S" ^ parens (gen_succ (m - 1) e)

and gen_path (p : path) (is_match : bool) (e : exp) =
  match p.it with   
    | RootP -> gen_exp is_match e
    | IdxP (path, exp) -> "lookup_total " ^ parens (gen_path path is_match e) ^ " " ^ parens (gen_exp is_match exp) 
    | SliceP (_path, _exp1, _exp2) -> "default_val" (* TODO *)
    | DotP (path, atom) -> gen_typ_name path.note ^ "__" ^ gen_atom atom ^ " " ^ parens (gen_path path is_match e)

let rec gen_premises (p : prem) =
  match p.it with
    | RulePr (id, _mixop, exp) -> gen_id id ^ " " ^ gen_exp false exp
    | IfPr exp -> gen_exp false exp
    | ElsePr -> ""
    | LetPr (_exp1, _exp2, _ids) -> "1"
    | IterPr (p, (iter, ids)) -> (match iter, ids with
      | Opt, [(i, _typ)] -> "Option.map " ^ parens ( "fun " ^ var_prefix ^ gen_id i ^ " => " ^ gen_premises p) ^ " " ^ parens (var_prefix ^ gen_id i) 
      | (List | List1 | ListN _), [(i, _typ)] -> "List.Forall " ^ parens ( "fun " ^ var_prefix ^ gen_id i ^ " => " ^ gen_premises p) ^ " " ^ parens (var_prefix ^ gen_id i) 
      | _, _ -> gen_premises p
    )

let rec _gen_id_from_premises (p : prem) =
  match p.it with
    | RulePr (_, _, e) -> gen_exp_id_used e
    | IfPr e -> gen_exp_id_used e
    | LetPr (_exp1, exp2, _ids) -> gen_exp_id_used exp2
    | ElsePr -> IdSet.empty
    | IterPr (p, _iterexp) -> _gen_id_from_premises p

let gen_relation_premises (premises : prem list) =
  let prems = List.filter (fun p -> p.it <> ElsePr) premises in
  let e = (match prems with
    | [] -> ""
    | _ -> " -> ") in
  String.concat " /\\ " (List.map gen_premises prems) ^ e

let gen_param (p : param) = 
  match p.it with
    | ExpP (id, typ) -> parens (var_prefix ^ gen_id id ^ " : " ^ gen_typ typ)
    | TypP id -> curly_parens (gen_id id)

let gen_inductive_inhabitance_proof (id : id) (typcases : typcase list) : string =
  "Global Instance Inhabited__" ^ gen_id id ^ " : Inhabited " ^ gen_id id ^ 
  let simple_constructors = List.filter (fun (_, (_, typ, _), _) -> typ.it = TupT []) typcases in
  match simple_constructors with
  | [] -> "(* FIXME: no inhabitant found! *) .\n" ^
          "  Admitted"
  | (m, _, _) :: _ -> " := { default_val := " ^ gen_id id ^ "__" ^ gen_mixop m ^ " }"

let gen_record_inhabitance_proof (id : id) (typfields : typfield list) : string =
  "Global Instance Inhabited_" ^ gen_id id ^ " : Inhabited " ^ gen_id id ^ " := \n" ^
  "{default_val := {|\n" ^
      String.concat "" (List.map (fun (a, _, _) -> 
        "  " ^ gen_id id ^ "__" ^ gen_atom a ^ " := default_val ;\n"
        ) typfields) ^ "|} }"

let gen_deftyp (binds : bind list) (args : arg list) (id : id) (d : deftyp) =
  match d.it with
    | AliasT typ -> "Definition " ^ gen_id id ^ gen_inductive_args binds args ^  " := " ^ gen_typ typ
    | StructT typfields -> "Record " ^ gen_id id ^ " := \n{" ^ "\t" ^
      String.concat "\n;\t" (List.map (fun (a, (_, t, _premises), _) -> 
        gen_id id ^ "__" ^ gen_atom a ^ " : " ^ gen_typ t
      ) typfields) ^ "\n}" ^ ".\n\n" ^
      gen_record_inhabitance_proof id typfields
    | VariantT typcases -> 
      let arg_names = match args with 
        | [] -> "" 
        | _ -> " " ^ String.concat " " (List.filter_map (gen_arg_name false) args)
      in 
      "Inductive " ^ gen_id id ^ " " ^ gen_inductive_args binds args ^  " : Type :=\n" ^ 
      String.concat "\n" (List.map (fun (m, (_, t, _premises), _) -> 
      "\t| " ^ gen_id id ^ "__" ^ gen_mixop m ^ gen_typ_args t ^ ": " ^
      gen_id id ^ arg_names) typcases) ^ ".\n\n" ^ gen_inductive_inhabitance_proof id typcases

let gen_param_id_used (param : param) =
  match param.it with
    | ExpP (id, _) -> Some (var_prefix ^ gen_id id)
    | _ -> None

let gen_def_params (params : param list) =
  let id_params = List.combine (List.init (List.length params) (fun i -> string_of_int i)) params in
  List.map (fun (i, p) -> (match p.it with
    | ExpP (id, typ) -> let var_i = var_prefix ^ (if (gen_id id) = "_" then i else gen_id id) in
      parens (var_i ^ " : " ^ gen_typ typ)
    | TypP id -> curly_parens (gen_id id)
  )) id_params

let gen_match_clause (params : param list) =
  match params with
    | [] -> ""
    | _ -> let id_params = List.combine (List.init (List.length params) (fun i -> string_of_int i)) params in
    let match_names = List.filter_map (fun (i, p) -> (match p.it with
    | ExpP (id, _) -> Some (var_prefix ^ (if (gen_id id) = "_" then i else gen_id id)) 
    | TypP _ -> None
  )) id_params in 
    "\tmatch " ^ parens (String.concat ", " match_names) ^ " with\n"

let gen_instance (id : id) (params : param list) (i : inst) =
  match i.it with
    | InstD (binds, args, deftyp) -> (match deftyp.it with
      | AliasT typ -> "\t\t| " ^ gen_match_args args ^ " => " ^ gen_typ typ
      | VariantT typcases -> 
      let inductive_name = gen_id id ^ "__" ^ String.concat "__" (List.map (fun b -> gen_bind_typ b) binds) 
      in 
      String.concat "\n" (List.map (fun (m, (_, t, _premises), _) -> 
      "\t| " ^ inductive_name ^ "__" ^ gen_mixop m ^ gen_typ_args t ^ ": " ^ gen_id id ^ " " ^ String.concat " " (List.filter_map gen_param_id_used params)) 
      typcases)
      | _ -> gen_deftyp binds args id deftyp
    )
let _get_instance_binds (i : inst) =
  match i.it with
    | InstD (binds, args, _) -> (binds, args)
    
let gen_instances (params : param list) (id : id) (insts : inst list) =
  let i = List.hd insts in
  match i.it with
  | InstD (_, _, deftyp) -> (match deftyp.it with 
    | AliasT _ -> "Definition " ^ gen_id id ^ 
    String.concat " " (gen_def_params params) ^ " :=\n"  ^
    gen_match_clause params ^ 
    String.concat "\n" (List.map (gen_instance id params) insts) ^
    "\n\tend"
    | StructT _ -> "" (* Should not happen *)
    | VariantT _ -> 
    "Inductive " ^ gen_id id ^ 
    String.concat " " (gen_def_params params) ^ ": Type :=\n" ^
    String.concat "\n" (List.map (gen_instance id params) insts)
  ) 
  
let gen_clause (c : clause) = 
  match c.it with
    | DefD (_binds, args, exp, _premises) -> "\t\t| " ^ gen_match_args args ^ " => " ^ gen_tup_exp exp

let gen_clauses (params : param list) (clauses : clause list) =
  match clauses with
    | [{it = DefD (_, _, exp, _); _}] when params == [] -> " := " ^ gen_exp false exp 
    | _ -> " :=\n" ^ gen_match_clause params ^ String.concat "\n" (List.map gen_clause clauses) ^ "\n\tend"

let gen_ids_used (binds : bind list) = 
  if binds == [] then "" else "forall " ^ String.concat " " (List.map gen_bind binds) ^ ", "

let gen_rule (id : id) (rule : rule) =
  match rule.it with
    | RuleD (rule_id, binds, mixop, exp, prems) -> let rule_id' = if String.empty <> rule_id.it then gen_id rule_id ^ "_" else "" in
    "\t| " ^ gen_id id ^ "__" ^ rule_id' ^ gen_mixop mixop ^ " : " ^ gen_ids_used binds ^ gen_relation_premises prems ^ gen_id id ^ " " ^ gen_exp false exp

let gen_rules (id : id) (rules : rule list) =
  String.concat "\n" (List.map (gen_rule id) rules)

let gen_axiom (id : id) (params : param list) (typ : typ) = 
  "Axiom " ^ func_prefix ^ gen_id id ^ " : forall " ^ String.concat " " (List.map gen_param params) ^ ", " ^ gen_typ typ

let is_inductive (d : def) =
  match d.it with
    | TypD _ |  RelD _ -> true
    | _ -> false

let is_not_hintdef (d : def) =
  match d.it with
    | HintD _ -> false
    | _ -> true

let rec gen_def (env : env) (is_recursive : bool) (d : def)=
  match d.it with
    | TypD (id, _params, [{it = InstD (binds, args, deftyp); _}]) -> gen_deftyp binds args id deftyp
    | TypD (id, params, insts) -> gen_instances params id insts
    | RelD (id, _mixop, typ, rules) -> "Inductive " ^ gen_id id ^ ": " ^ gen_relation_param_types typ ^ " -> Prop := \n" ^ gen_rules id rules
    | DecD (id, params, typ, clauses) -> let prefix = if is_recursive then "Fixpoint " else "Definition " in 
      if clauses == [] then
        (gen_axiom id params typ)
      else 
        (prefix ^ func_prefix ^ gen_id id ^ " "
        ^ String.concat " " (gen_def_params params)
        ^ ": " ^ gen_typ typ
        ^ gen_clauses params clauses)
    | RecD defs -> (match defs with
        | [] -> ""
        | [d] -> gen_def env true d
        | d :: ds -> let inductive_word = if is_inductive d then "with\n\n" else "" in
          gen_def env true d ^ "\n\n" ^ inductive_word ^ String.concat "\n\n" (List.map (gen_def env true) ds))  
    | _ -> ""

let gen_script (env : env) (il : script) =
  String.concat ".\n\n" (List.map (gen_def env false) (List.filter is_not_hintdef il)) ^ ".\n"

let gen_string (il : script) =
  let env = Case.get_case_env il in 
  print_env env;
  "From Coq Require Import String List Unicode.Utf8.\n" ^
  "Import ListNotations.\n" ^
  "Require Import NArith.\n" ^
  "Require Import Arith.\n" ^
  "Require Import BinNat.\n" ^
  "Require Import FloatClass.\n" ^
  "Require Import PrimFloat.\n" ^
  "Require Import SpecFloat.\n" ^
  "Require Import FloatOps.\n" ^
  "Require Import FloatAxioms.\n" ^
  "Require Import FloatLemmas.\n" ^
  "Class Inhabited (T: Type) := { default_val : T }.\n\n" ^
  "Definition lookup_total {T: Type} {_: Inhabited T} (l: list T) (n: nat) : T :=\n" ^
  "\tList.nth n l default_val.\n\n" ^
  "Definition the {T : Type} {_ : Inhabited T} (arg : option T) : T :=\n" ^
	"\tmatch arg with\n" ^
	"\t\t| Some a => a\n" ^
	"\t\t| None => default_val\n" ^
	"\tend.\n\n" ^
  "Fixpoint list_update {α: Type} (l: list α) (n: nat) (y: α): list α :=\n" ^
  "\tmatch l, n with\n" ^
  "\t\t| nil, _ => nil\n" ^
  "\t\t| x :: l', 0 => y :: l'\n" ^
  "\t\t| x :: l', S n => x :: list_update l' n y\n" ^
  "\tend.\n\n" ^
  "Definition list_extend {α: Type} (l: list α) (y: α): list α :=\n" ^
  "\ty :: l.\n\n" ^
  "Definition option_to_list {α : Type} (o : option α): list α :=\n" ^
  "\tmatch o with\n" ^
  "\t\t| None => nil\n" ^
  "\t\t| Some a => cons a nil\n" ^
  "\tend.\n\n" ^
  "Global Instance Inh_unit : Inhabited unit := { default_val := tt }.\n\n" ^
  "Global Instance Inh_nat : Inhabited nat := { default_val := O }.\n\n" ^
  "Global Instance Inh_list {T: Type} : Inhabited (list T) := { default_val := nil }.\n\n" ^
  "Global Instance Inh_option {T: Type} : Inhabited (option T) := { default_val := None }.\n\n" ^
  "Global Instance Inh_prod {T1 T2: Type} {_: Inhabited T1} {_: Inhabited T2} : Inhabited (prod T1 T2) := { default_val := (default_val, default_val) }.\n\n" ^
  "\n" ^
  "(* Generated Code *)\n" ^
  gen_script env il


let gen_file file il =
  let coq_code = gen_string il in
  let oc = Out_channel.open_text file in
  Fun.protect (fun () -> Out_channel.output_string oc coq_code)
    ~finally:(fun () -> Out_channel.close oc)

