...
let () =
  ...
  try
    ...
    let il = if not (!pass_sub || !target = Haskell || !target = Lean4 || !target = Coq) then il else
    ...
    let il = if not (!pass_totalize || !target = Lean4 || !target = Coq) then il else
    ...
    let il = if not (!pass_unthe || !target = Lean4 || !target = Coq) then il else
    ...
    let il = if not (!pass_sideconditions || !target = Lean4 || !target = Coq) then il else
    ...
    let il = if not (!pass_else_elim || !target = Lean4 || !target = Coq) then il else
