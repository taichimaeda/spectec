...
let rec valid_iter env env' iter =
  match iter with
  | Opt | List | List1 -> ()
  | ListN (e, e_opt) ->
    valid_exp env e (NatT $ e.at);
    Option.iter (fun e2 -> valid_exp env' e2 (NatT $ e2.at)) e_opt
...
and valid_typ env t =
  ...
  | IterT (t1, iter) ->
    ...
    | _ -> valid_typ env t1; valid_iter env (new_env ()) iter
