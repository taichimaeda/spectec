...
module Run (T : TRAVERSAL) = struct
  ...
  and traverse_path' p acc =
    ...
    | RootP -> (RootP, acc)
    | IdxP (p, e) ->
        let (p', e'), acc' = traverse2 (traverse_path p) (traverse_exp e) acc in
        (IdxP (p', e'), acc')
    | SliceP (p, e1, e2) ->
        let (p', e1', e2'), acc' =
...
let traverse_script (type a) (module T : TRAVERSAL with type acc = a) s acc =
  let module R = Run (T) in
  let s', acc' = (traverse_list R.traverse_def) s acc in
  (s', acc')
